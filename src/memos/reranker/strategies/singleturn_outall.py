...
class SingleTurnOutAllStrategy(BaseRerankerStrategy):
    """
        ...
        for item in graph_results:
            ...
            for i in range(0, len(sources), 2):
                ...
                if user_content or assistant_content:  # Only add non-empty pairs
                    pair_index = i // 2
                    if user_content not in sources_dialogue_pairs:
                        sources_dialogue_pairs.append(user_content)
                        tracker.add_dialogue_pair(
                            item.id, pair_index, user_msg, assistant_msg, memory or "", chat_time, split_source=True
                        )
                    else:
                        tracker.add_dialogue_pair(
                            item.id, pair_index, user_msg, assistant_msg, memory or "", chat_time
                        )
        documents = tracker.get_documents_for_ranking(concat_memory=False)
        ...
        **kwargs,
        ...
        for idx, score in zip(ranked_indices, scores, strict=False):
            ...
            if dialogue_pair and (dialogue_pair.memory_id in original_items):
                ...
                reconstructed_item.memory = dialogue_pair.combined_text
