...
# base_model = "bge-reranker-v2-m3"
base_model = "Qwen3-Reranker-4B"

class RerankerEvaluator:
    def __init__(self, rerank_url: str = "http://openai.com/v1/rerank", batch_size: int = 50):
    ...
    def call_reranker(self, query: str, documents: List[str]) -> List[Dict]:
        """call reranker API
        Args:
            query: str, the query
            documents: List[str], the documents
        Returns:
            List[Dict], the reranker results
        """
        payload = {
            "model": base_model,
            "query": query,
        ...
        try:
            ...
            print(f"Reranker API调用失败: {e}")
            return []
    
    def call_reranker_batched(self, query: str, documents: List[str], batch_size: int = 50) -> List[Dict]:
        """分批调用reranker API并合并结果
        Args:
            query: str, the query
            documents: List[str], the documents
            batch_size: int, 每批处理的文档数量
        Returns:
            List[Dict], 合并后的reranker results，格式与call_reranker相同
        """
        if len(documents) <= batch_size:
            # 如果文档数量不超过批次大小，直接调用原方法
            return self.call_reranker(query, documents)
        
        all_results = []
        total_batches = math.ceil(len(documents) / batch_size)
        
        print(f"文档总数: {len(documents)}, 分为 {total_batches} 批处理，每批 {batch_size} 个文档")
        ...
        for batch_idx in range(total_batches):
            ...
            print(f"处理第 {batch_idx + 1}/{total_batches} 批 (文档 {start_idx}-{end_idx-1})")
            
            # 调用reranker API
            batch_results = self.call_reranker(query, batch_documents)
            ...
            for result in batch_results:
                result['index'] = result['index'] + start_idx
                result['batch_idx'] = batch_idx  # 添加批次信息用于调试
        ...
        for result in all_results:
            if 'batch_idx' in result:
                del result['batch_idx']
        
        print(f"分批处理完成，共处理 {len(all_results)} 个文档")
        return all_results
    ...
    def evaluate_single_question(self, data: Dict, mode: str = 'single', n_turns: int = 3, use_batched: bool = True) -> Dict:
        ...
        # 调用reranker
        if use_batched:
            rerank_results = self.call_reranker_batched(query, documents, self.batch_size)
        else:
            rerank_results = self.call_reranker(query, documents)
        
        if not rerank_results:
            return {'error': 'Reranker API failed'}
        
        # 提取排序后的文档索引
        ranking = [result['index'] for result in rerank_results]
...
def main():
    ...
    # 创建评估器，设置批次大小为30
    evaluator = RerankerEvaluator(batch_size=10)
    ...
    # # 可选：对比不使用分批处理的结果（仅在文档数量较少时使用）
    # print("\n评估单轮对话模式 (不使用分批处理，仅用于对比)...")
    # try:
    #     single_results_no_batch = evaluator.evaluate_dataset(dataset[:10], mode='single', use_batched=False)
    #     print("不使用分批处理的结果:")
    #     for metric, value in single_results_no_batch['metrics'].items():
    #         print(f"{metric}: {value:.4f}")
    # except Exception as e:
    #     print(f"不使用分批处理时出错: {e}")
    ...
    results = {
        ...
        'evaluation_config': {
            ...
            'rerank_url': evaluator.rerank_url,
            'model': base_model
        }
    }
    prefix = datetime.now().strftime('%Y%m%d_%H%M%S') + f"_{base_model}_batched"
