"""
Step 1: Ingest dialogues + search update memories + QA responses.

This must be a single step because the original eval_memos.py processes each
session serially: ingest dialogue -> search updated memories -> answer questions.
The search/QA depends on the memory state after each session's ingestion, so
they cannot be separated into independent pipeline stages.
"""
...
def add_dialogue(dialogue, user_id, conv_id):
    ...
    formatted_dialogue = [
        {
            ...
            "chat_time": datetime.strptime(turn["timestamp"], date_format)
    ...
    for i in range(0, len(formatted_dialogue), batch_num):
        ...
        memories.extend([item["memory"] for item in response["data"]])
...
def main(data_path, version, lib, top_k, pref_top_k, max_workers):
    save_path = f"results/halumem/{lib}-{version}/"
    os.makedirs(save_path, exist_ok=True)

    output_file = os.path.join(save_path, "ingestion_results.jsonl")
    tmp_dir = os.path.join(save_path, "tmp")
    os.makedirs(tmp_dir, exist_ok=True)
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {}
        for _idx, user_data in enumerate(iter_jsonl(data_path), 1):
            uuid_val = user_data["uuid"]
            future = executor.submit(process_user, user_data, top_k, pref_top_k, save_path, version)
            futures[future] = uuid_val
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="HaluMem Ingestion + Search + QA")
    parser.add_argument("--data", type=str, required=True, help="Path to HaluMem jsonl data file")
