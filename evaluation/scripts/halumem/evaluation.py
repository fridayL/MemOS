...
def process_user(idx: int, user_data: dict, max_workers: int = 10):
    ...
    for sid, session in enumerate(user_data["sessions"]):
        ...
        for memory in extract_memories:
            new_memory = {"uuid": uuid, "ssession_id": sid, "memory_content": memory}
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for memory, extract_memories_str in memory_integrity_inputs:
            ...
            future = executor.submit(
                evaluation_for_memory_integrity, extract_memories_str, memory["memory_content"]
            )
        ...
        for future in tqdm(
            ...
            total=len(futures),
            desc=f"Memory Integrity Evaluation([{idx}]{user_name})",
        ):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(
            as_completed(futures),
            total=len(futures),
            desc=f"Memory Accuracy Evaluation([{idx}]{user_name})",
        ):
            memory = futures[future]
            try:
                result = future.result()
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(
            as_completed(futures),
            total=len(futures),
            desc=f"Memory Update Evaluation([{idx}]{user_name})",
        ):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(
            as_completed(futures),
            total=len(futures),
            desc=f"Question-Answering Evaluation([{idx}]{user_name})",
        ):
            qa = futures[future]
            try:
                result = future.result()
...
def aggregate_eval_results(eval_results):
    ...
    for item in eval_results["memory_integrity_records"]:
        ...
        if item["memory_source"] != "interference":
            ...
            memory_integrity_weighted_scores += (
                0.5 * item["memory_integrity_score"] * item["importance"]
            )
    ...
    eval_results["overall_score"]["memory_integrity"]["recall(all)"] = (
        memory_integrity_scores / memory_integrity_num
    )
    eval_results["overall_score"]["memory_integrity"]["recall(valid)"] = (
        memory_integrity_scores / memory_integrity_valid_num
    )
    eval_results["overall_score"]["memory_integrity"]["weighted_recall(all)"] = (
        memory_integrity_weighted_scores / memory_integrity_weighted_num
    )
    ...
    eval_results["overall_score"]["memory_integrity"]["memory_valid_importance_sum"] = (
        memory_integrity_weighted_valid_num
    )
    eval_results["overall_score"]["memory_integrity"]["memory_importance_sum"] = (
        memory_integrity_weighted_num
    )
    eval_results["overall_score"]["memory_integrity"]["memory_valid_num"] = (
        memory_integrity_valid_num
    )
    ...
    eval_results["overall_score"]["memory_accuracy"]["interference_accuracy(all)"] = (
        interference_memory_scores / interference_memory_num
    )
    eval_results["overall_score"]["memory_accuracy"]["interference_accuracy(valid)"] = (
        interference_memory_scores / interference_memory_valid_num
    )
    eval_results["overall_score"]["memory_accuracy"]["interference_memory_valid_num"] = (
        interference_memory_valid_num
    )
    eval_results["overall_score"]["memory_accuracy"]["interference_memory_num"] = (
        interference_memory_num
    )
    ...
    eval_results["overall_score"]["memory_accuracy"]["target_accuracy(all)"] = (
        target_memory_accuracy_scores / target_memory_accuracy_num
    )
    eval_results["overall_score"]["memory_accuracy"]["target_accuracy(valid)"] = (
        target_memory_accuracy_scores / target_memory_accuracy_valid_num
    )
    eval_results["overall_score"]["memory_accuracy"]["target_memory_valid_num"] = (
        target_memory_accuracy_valid_num
    )
    eval_results["overall_score"]["memory_accuracy"]["target_memory_num"] = (
        target_memory_accuracy_num
    )
    eval_results["overall_score"]["memory_accuracy"]["weighted_accuracy(all)"] = (
        memory_accuracy_weighted_scores / memory_accuracy_num
    )
    eval_results["overall_score"]["memory_accuracy"]["weighted_accuracy(valid)"] = (
        memory_accuracy_weighted_scores / memory_accuracy_valid_num
    )
    ...
    eval_results["overall_score"]["memory_update"]["update_memory_valid_num"] = (
    ...
    eval_results["overall_score"]["question_answering"]["correct_qa_ratio(all)"] = (
        correct_qa_num / qa_num
    )
    eval_results["overall_score"]["question_answering"]["correct_qa_ratio(valid)"] = (
        correct_qa_num / qa_valid_num
    )
    ...
    eval_results["overall_score"]["question_answering"]["omission_qa_ratio(all)"] = (
        omission_qa_num / qa_num
    )
    eval_results["overall_score"]["question_answering"]["omission_qa_ratio(valid)"] = (
        omission_qa_num / qa_valid_num
    )
    ...
    for item in eval_results["memory_integrity_records"]:
        ...
        eval_results["overall_score"]["memory_type_accuracy"][item["memory_type"]][
            "memory_integrity_acc"
        ] += score
    ...
    for key in eval_results["overall_score"]["memory_type_accuracy"]:
        eval_results["overall_score"]["memory_type_accuracy"][key]["memory_integrity_acc"] = (
            eval_results["overall_score"]["memory_type_accuracy"][key]["memory_integrity_acc"]
            / eval_results["overall_score"]["memory_type_accuracy"][key]["total_num"]
        )
...
def main(frame: str, version: str = "default", user_num: int = 20, max_workers: int = 10):
    ...
    for file_name in os.listdir(tmp_dir):
        ...
        eval_results["memory_accuracy_records"].extend(
            user_result.get("memory_accuracy_records", [])
        )
        eval_results["memory_integrity_records"].extend(
            user_result.get("memory_integrity_records", [])
        )
        ...
        eval_results["question_answering_records"].extend(
            user_result.get("question_answering_records", [])
        )
