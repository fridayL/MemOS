...
def process_user(user_data, top_k, pref_top_k, save_path, version):
    ...
    new_user_data = {"uuid": user_data["uuid"], "user_name": user_name, "sessions": []}
    ...
    try:
        ...
        for session in tqdm(sessions, total=len(sessions), desc=f"Processing user {user_name}"):
            ...
            result, duration_ms = add_dialogue(
                dialogue=dialogue, user_id=user_name, conv_id=conv_id
            )
            ...
            for memory in new_session["memory_points"]:
                ...
                _, memories_from_system, duration_ms = search_memory(
                    query=memory["memory_content"], user_id=user_name, top_k=10, pref_top_k=2
                )
            ...
            for qa in session["questions"]:
                context, _, duration_ms = search_memory(
                    query=qa["question"], user_id=user_name, top_k=top_k, pref_top_k=pref_top_k
                )
...
if __name__ == "__main__":
    data_path = "/mnt/workspace/lichunyu/MemOS/tmp/HaluMem-Medium (1).jsonl"
    version = "default"
    top_k = 20
    max_workers = 10

    main(data_path=data_path, version=version, top_k=top_k, max_workers=max_workers)
